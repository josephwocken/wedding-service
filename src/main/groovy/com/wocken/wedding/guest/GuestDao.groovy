...
class GuestDao {
    ...
    Guest getGuestById(long guestId) {
        ...
        while (rs.next()) {
            try {
                ...
                Guest guest = new Guest(retrievedGuestId, name)
                println(guest)
                return guest
            } finally {
                statement.close()
    ...
    void createGuest(Guest guest) {
        ...
        Statement statement = connection.createStatement()
        try {
            statement.executeUpdate("INSERT INTO guest (invitation_id, name) VALUES (2, '$guestName')")
        } finally {
            statement.close()
        }
    }
    ...
    private Guest mapToGuest(ResultSet rs) {
        long retrievedGuestId = rs.getLong("id") //this might be auto-generated by the db
        String name = rs.getString("name")
        return new Guest(retrievedGuestId, name)
    }
}
